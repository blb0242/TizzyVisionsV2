...
export default function MusicVideos() {
    ...
    const handleOpen = (playlist, video) => { 
        console.log(playlist)
        const body = (
            <ModalComponent playlist={playlist} playlistName={playlist.title} video={video} />
        );
        setModal(body)
        setOpen(true);
    };
    ...
    useEffect(() => {
        fetch("https://tizzy-visions-v2.herokuapp.com/api/playlists")
            ...
            .then(ytPlaylists => {
                ...
                let checkTitle = (list) => {
                    return list.title ? list.title.includes("Music Videos") : null;
                }
                let musicVideos = ytPlaylists.filter(checkTitle)
                setVideos(musicVideos[0].videos)
            })
            .catch(console.error());
    }, [])
            ...
            <GridList cellHeight={100} spacing={5} className={classes.gridList}>
                {videos.map((video, index) => (
                    <GridListTile key={video.poster} cols={index === 0 || index === 3 || index === 6 ? 2 : 1} rows={index === 0 || index === 3 || index === 6 ? 2 : 1}>
                        <img onClick={() => handleOpen(videos, index)} src={video.poster} alt={video.title} />
                        <GridListTileBar
                            title={video.title}
